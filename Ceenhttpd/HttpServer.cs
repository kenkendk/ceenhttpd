			...
			using (var ssl = new SslStream(s, false))
			{
				...
				catch (Exception aex)
				{
					if (Config.Logger != null)
						try { await Config.Logger.LogRequest(new HttpRequest(client.Client.RemoteEndPoint, logtaskid, null), null, aex, DateTime.Now, new TimeSpan()); }
						catch { }
				...
				using (var bs = new BufferedStreamReader(stream))
				{
					do
					{
						...
						try
						{
							...
							// Process the request
							if (!await Config.Router.Process(cur, resp))
						...
						if (Config.Logger != null)
							await Config.Logger.LogRequest(cur, resp, null, started, DateTime.Now - started);
			...
			catch (Exception ex)
			{
				...
				if (resp != null)
				{
					if (!resp.HasSentHeaders)
					{
						...
						resp.StatusMessage = HttpStatusMessages.DefaultMessage(HttpStatusCode.InternalServerError);
					}
				...
				{
					if (Config.Logger != null)
						await Config.Logger.LogRequest(cur, resp, ex, started, DateTime.Now - started);
				}
				catch
				{
				}
