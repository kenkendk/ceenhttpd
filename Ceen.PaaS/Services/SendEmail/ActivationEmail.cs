    ...
    public static class ActivationEmail
    {
        ...
        public static async Task SendAsync(string name, string email, string activationcode, string language, string requestIP)
        {
            ...
            await DatabaseInstance.GetInstance().RunInTransactionAsync(db =>
            {
                ...
                markdown = Services.TextHelper.GetTextFromDb(db, TextConstants.ActivationEmailBody, language);
                subject = Services.TextHelper.GetTextFromDb(db, TextConstants.ActivationEmailSubject, language);
